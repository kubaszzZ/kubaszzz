...
char stu_menu()//学生菜单
{
	...
	if(1==option&&0==flag) return option;
	else if(1==option&&1==flag) printf("已登录");
	while(1!=option&&0==flag)
	{
		if(5==option) return option;
		printf("请先登录：");
		scanf("%d",&option);
	...
	return option;
	/*if(2<=option&&4>=option&&1==flag)
	{
		return option;
	}
	else if(2<=option&&4>=option&&0==flag)
	{
		printf("请先登录：");
		scanf("%d",&option);
	}
