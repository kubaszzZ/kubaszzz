...
void add_tea()
{
	...
	for(int i=0;;i++)
	{
		...
		if(0==tea[i].tea_id)
		{
			...
			do{
				scanf("%s",tea[i].tea_name);
				if(20 < strlen(tea[i].tea_name))
				{
					printf("输入的姓名不合法!请重新输入:");
				}
			}while(20 < strlen(tea[i].tea_name));			//	最大姓名长度
			printf("输入添加教师性别:");							//	获取教师性别
			do{
				scanf("%d",&tea[i].tea_sex);
				if(1!=tea[i].tea_sex && 2!=tea[i].tea_sex)
				{
					printf("输入的性别不合法!请重新输入:");
				}
			}while(1 != tea[i].tea_sex && 2 != tea[i].tea_sex);
			printf("输入添加教师工号:");							
			scanf("%d",&tea[i].tea_id);
			break;
		}
	}
	printf("添加成功\n");
...
{	
	for(int i=0;i<MAX;i++)
	{
		...
		{
			printf("-%d-%5s--%d--%4d--%4s--%hhd\n",i, tea[i].tea_name, tea[i].tea_sex, tea[i].tea_id, tea[i].tea_password,tea[i].tea_error);
		}		
	}
	usleep(1.5*1000000);
}
...
void show_dieteacher()
{
	for(int i=0;i<MAX;i++)
	{
		if(0 != tea[i].tea_sex && 0 == tea[i].tea_id)	//	用ID和性别来判定离职教师
		{
			printf("%s\n", tea[i].tea_name);	
		}
	}
