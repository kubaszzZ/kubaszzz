...
char tea_menu(void)
{
	...
	for(int t=0;t<100;t++)
	{
		/*if(0==t||(t>0&&tea[t-1].tea_sex>0))*/fscanf(fpx,"%s %d %d %s %hhd", tea[t].tea_name,&tea[t].tea_sex,&tea[t].tea_id,tea[t].tea_password,&tea[t].tea_error);
	}
	fclose(fpx);
	int i=0;
	char names[20];
	...
	while(0!=strcmp(key,tea[i].tea_password))
	{
		if(tea[i].tea_error==4) 
		{
			fpx=fopen("OLteacher.txt","w");
			for(int t=0;t<100;t++)
			{
				if(tea[t].tea_sex)fprintf(fpx,"%s %d %d %s %hhd\n",tea[t].tea_name,tea[t].tea_sex,tea[t].tea_id,tea[t].tea_password,tea[t].tea_error);
			}
			fclose(fpx);
			return 9;
		}
		tea[i].tea_error ++;
		printf("\n密码错误,请重试%hhd:",tea[i].tea_error);
...
void del_stu()
{
	...
	{
		if(student_count-1==i && (0!=strcmp(names,stu[i].stu_name)||stu[i].stu_id!=atoi(names)) )
		{
			printf("未能找到相关学生信息\n");
		}
		else if(0<stu[i].stu_sex && (0==strcmp(names,stu[i].stu_name)||stu[i].stu_id==atoi(names)) )
		{
			fprintf(fdel,"%s\n",stu[i].stu_name);
			stu[i].stu_sex=0;
...
/*	fopen("student.txt","w");
	for(i=0;i<student_count;i++)
	{
		if(stu[i].stu_sex==0) continue;
	 	if(stu[i].stu_sex>0) fprintf(fp,"%s %d %d %s %hd %hd %hd %hhd\n",stu[i].stu_name,stu[i].stu_sex,stu[i].stu_id,stu[i].stu_password,stu[i].chinese_score,stu[i].math_score,stu[i].english_score,stu[i].stu_error);
...
void show_allstu()
{
	...
	/*else 
	{
		FILE* fpx = fopen("overlord.txt","r");
		fscanf(fpx,"%d",&out);
		for(i=0;i<out;i++)
		{
			fscanf(fpx,"%s %d %d %s %hd %hd %hd %hhd",stu[i].stu_name,&stu[i].stu_sex,&stu[i].stu_id,stu[i].stu_password,&stu[i].chinese_score,&stu[i].math_score,&stu[i].english_score,&stu[i].stu_error);
		}
		fclose(fpx);
	}*/
	for(i=0;i<student_count;i++)
