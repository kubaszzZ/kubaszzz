...
void add_tea()
{
	...
	for(;;)
	{
		...
		if(0==tea[i].tea_id)
		{//判断结构体中的空位从文件中读取
			scanf("%s %d %d %s", tea[i].tea_name, &tea[i].tea_sex, &tea[i].tea_id, tea[i].tea_password);//输入添加的教师
			int str_l=0;
	...
	if(tea_id2==tea_id1)
	{
		...
		for(i=0;;)
		{
			if(tea[i].tea_id==tea_id2)
			{
				...
				for(;j<MAX;)
				{
					fscanf(frotea,"%s %d %d %s\n", tea[j].tea_name, &tea[j].tea_sex, &tea[j].tea_id, tea[j].tea_password);//获取文件信息
					if(tea_id2==tea[j].tea_id)
					{
						...
						printf("--%d--\n", str_l);
						fseek(frotea,-str_l,1);
						fprintf(frotea,"%s %d %3d %s", tea[j].tea_name, tea[j].tea_sex, tea[j].tea_id, tea[j].tea_password);//只改变id为0,%3d可以为ID的最大位数
						//完全删除(未实现)					
						break;
					}
					j++;
