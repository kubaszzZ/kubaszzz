...
void add_tea()
{
	...
	for(;;i++)
	{
		...
		fscanf(frotea,"%s %d %d %s %hhd\n", tea[i].tea_name, &tea[i].sex_tea, &tea[i].tea_id, tea[i].tea_password,&tea[i].tea_error);
		//↑从文件中获取信息--错误次数
		if(0==tea[i].tea_id)
		{//判断结构体中的空位从文件中读取
			scanf("%s %d %d", tea[i].tea_name, &tea[i].sex_tea, &tea[i].tea_id);//输入添加的教师--错误次数
			int str_l=0;
			...
			fprintf(frotea,"%s %d %d %s %2hhd", tea[i].tea_name, tea[i].sex_tea,tea[i].tea_id, tea[i].tea_password,tea[i].tea_error);
			//--错误次数
			break;
		}
	}
	...
	if(tea_id2==tea_id1)
	{
		...
		for(i=0;;i++)
		{
			...
			if(tea[i].tea_id==tea_id2)
			{
				...
				for(;j<MAX;j++)
				{
					fscanf(frotea,"%s %d %d %s %hhd\n", tea[j].tea_name, &tea[j].sex_tea, &tea[j].tea_id, tea[j].tea_password,&tea[i].tea_error);//获取文件信息----错误信息
					...
					if(tea_id2==tea[j].tea_id)
					{
						...
						printf("--%d--\n", str_l);
						fseek(frotea,-str_l,1);
						fprintf(frotea,"%s %d %d %s %hhd\n", tea[j].tea_name, tea[j].sex_tea, tea[j].tea_id, tea[j].tea_password,tea[j].tea_error);//只改变id为0,%3d可以为ID的最大位数---错误次数(从fseek中减去位数)
						//完全删除(未实现)					
						break;
					}
				}
